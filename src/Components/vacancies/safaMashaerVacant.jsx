...
import SafaMashaerAddRooms from "./SafaMashaerAddRooms";
import SafaMashaerEditRooms from "./SafaMashaerEditRooms";
...
const SafaMashaerVacant = (props) => {
  ...
  useEffect(() => {
    const vacantList = async () => {
      setIsLoading(true);
      await axios.get(`${url}/safamashaer/vacant`).then(async (res) => {
        setVacant(res.data);
        setIsLoading(false);
      });
    };
  ...
  useEffect(() => {
    const filterList = async () => {
      setLoading(true);
      await axios.get(`${url}/safamashaer/filter`).then(async (res) => {
  ...
  useEffect(() => {
    const alameiaRoomList = async () => {
      setLoading(true);
      await axios.get(`${url}/safamashaerrooms/list`).then(async (res) => {
    ...
    <div className="dataTableHome">
      ...
      <div>
        ...
        >
          ...
          <a
            style={{ fontSize: "13px" }}
            href={`${url}/safamashaervacant/file`}
...
export default SafaMashaerVacant;
