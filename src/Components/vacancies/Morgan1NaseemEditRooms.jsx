...
function Morgan1NaseemEditRooms() {
  function Loading() {
    return (
  ...
  const fetchSearchResults = async (event) => {
    ...
    axios
      .get(`${api}/searchvacant/morgan1naseem?q=${query}`)
      .then(async (res) => {
        setData(res.data);
        setQuery("");
      })
  ...
  const updateResults = async (e) => {
    ...
    setIsLoading(true);
    await axios
      .put(`${api}/morgan1naseemvacant/update/` + id, item)
      .then((response) => {
        setShow(false);
        Swal.fire({
          position: "center",
...
export default Morgan1NaseemEditRooms;
