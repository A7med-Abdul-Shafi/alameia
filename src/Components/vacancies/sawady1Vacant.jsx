...
import Sawady1AddRooms from "./Sawady1AddRooms";
import Sawady1EditRooms from "./Sawady1EditRooms";
...
const Sawady1Vacant = (props) => {
  ...
  useEffect(() => {
    const vacantList = async () => {
      setIsLoading(true);
      await axios.get(`${url}/sawady1/vacant`).then(async (res) => {
        setVacant(res.data);
        setIsLoading(false);
      });
    };
  ...
  useEffect(() => {
    const filterList = async () => {
      setLoading(true);
      await axios.get(`${url}/sawady1/filter`).then(async (res) => {
  ...
  useEffect(() => {
    const alameiaRoomList = async () => {
      setLoading(true);
      await axios.get(`${url}/sawady1rooms/list`).then(async (res) => {
