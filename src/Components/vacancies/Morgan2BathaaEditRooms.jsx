...
function Morgan2BathaaEditRooms() {
  function Loading() {
    return (
  ...
  const fetchSearchResults = async (event) => {
    ...
    axios
      .get(`${api}/searchvacant/morgan2bathaa?q=${query}`)
      .then(async (res) => {
        setData(res.data);
        setQuery("");
      })
  ...
  const updateResults = async (e) => {
    ...
    setIsLoading(true);
    await axios
      .put(`${api}/morgan2bathaavacant/update/` + id, item)
      .then((response) => {
        setShow(false);
        Swal.fire({
          position: "center",
...
export default Morgan2BathaaEditRooms;
