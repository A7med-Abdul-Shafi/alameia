...
import Morgan2BathaaAddRooms from "./Morgan2BathaaAddRooms";
import Morgan2BathaaEditRooms from "./Morgan2BathaaEditRooms";
import api from "../../customApi";
function Morgan2BathaaVacant(props) {
  ...
  useEffect(() => {
    const vacantList = async () => {
      setIsLoading(true);
      await axios.get(`${api}/morgan2bathaa/vacant`).then(async (res) => {
        setVacant(res.data);
        setIsLoading(false);
      });
    };
  ...
  useEffect(() => {
    const filterList = async () => {
      setLoading(true);
      await axios.get(`${api}/morgan2bathaa/filter`).then(async (res) => {
  ...
  useEffect(() => {
    const alameiaRoomList = async () => {
      setLoading(true);
      await axios.get(`${api}/morgan2bathaarooms/list`).then(async (res) => {
  ...
  return (
    <div className="dataTableHome">
      <div style={{ display: "flex", justifyContent: "space-around" }}>
        <Morgan2BathaaAddRooms />
        <Morgan2BathaaEditRooms />
      </div>
      <hr />
      <div>
        <div
        ...
        >
          ...
          <a
            style={{ fontSize: "13px" }}
            href={`${api}/morgan2bathaavacant/file`}
...
export default Morgan2BathaaVacant;
