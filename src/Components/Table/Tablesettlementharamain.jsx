...
function TableHaramain() {
      ...
      formatter: (listofdatabase, value, row) => {
        return (
          <div
            ...
            onClick={() => navigate(`/dashboard/single1/${value.id}`)}
          >
            <svg
  ...
  const { data, isLoading, isError, refetch } = useQuery(
    ["haramain"],
    async () => {
      return await axios.get(`${api}/haramain`).then((res) => res.data);
    }
  );
  if (isError) {
...
export default React.memo(TableHaramain);
