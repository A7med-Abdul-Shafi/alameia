...
function TableEwaaa() {
      ...
      formatter: (listofdatabase, value, row) => {
        return (
          <div
            ...
            onClick={() => navigate(`/dashboard/single2/${value.id}`)}
          >
            <svg
  ...
  const { data, isLoading, isError, refetch } = useQuery(
    ["ewaaa"],
    async () => {
      return await axios.get(`${api}/ewaaa`).then((res) => res.data);
    }
  );
  if (isError) {
...
export default React.memo(TableEwaaa);
