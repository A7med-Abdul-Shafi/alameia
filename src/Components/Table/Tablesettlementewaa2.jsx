...
function TableEwaab() {
      ...
      formatter: (listofdatabase, value, row) => {
        return (
          <div
            ...
            onClick={() => navigate(`/dashboard/single3/${value.id}`)}
          >
            <svg
  ...
  const { data, isLoading, isError, refetch } = useQuery(
    ["ewaab"],
    async () => {
      return await axios.get(`${api}/ewaab`).then((res) => res.data);
    }
  );
  if (isError) {
...
export default React.memo(TableEwaab);
