...
function TableWaqf() {
      ...
      formatter: (listofdatabase, value, row) => {
        return (
          <div
            ...
            onClick={() => navigate(`/dashboard/singlewaqf/${value.id}`)}
          >
            <svg
  ...
  const { data, isLoading, isError, refetch } = useQuery(["waqf"], async () => {
    return await axios.get(`${api}/waqf`).then((res) => res.data);
  });
  if (isError) {
...
export default React.memo(TableWaqf);
