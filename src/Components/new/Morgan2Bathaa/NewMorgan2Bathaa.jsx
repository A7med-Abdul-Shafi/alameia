...
function NewMorganBathaa() {
  ...
  useEffect(() => {
    const projects = async () => {
      await axios.get(`${api}/morgan2bathaa/in_reason`).then((res) => {
  ...
  const addResults = async (e) => {
    ...
    setIsLoading(true);
    await axios
      .post(`${api}/morgan2bathaa`, addedData)
      .then((response) => {
        Swal.fire({
          position: "center",
          icon: "success",
...
export default NewMorganBathaa;
