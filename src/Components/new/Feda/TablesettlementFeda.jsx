...
function TableFeda() {
      ...
      formatter: (listofdatabase, value, row) => {
        return (
          <div
            ...
            onClick={() => navigate(`/dashboard/singlefeda/${value.id}`)}
          >
            <svg
  ...
  const { data, isLoading, isError, refetch } = useQuery(["feda"], async () => {
    return await axios.get(`${api}/feda`).then((res) => res.data);
  });
  if (isError) {
...
export default React.memo(TableFeda);
