...
function NewMorganNaseem() {
  ...
  useEffect(() => {
    const projects = async () => {
      await axios.get(`${api}/morgan1naseem/in_reason`).then((res) => {
  ...
  const addResults = async (e) => {
    ...
    setIsLoading(true);
    await axios
      .post(`${api}/morgan1naseem`, addedData)
      .then((response) => {
        Swal.fire({
          position: "center",
          icon: "success",
...
export default NewMorganNaseem;
